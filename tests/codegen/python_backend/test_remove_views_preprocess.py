...
def test_symbolic_nested_flatten_views_python_backend():
    ...
    A rank-changing symbolic view cannot always be composed into a NestedSDFG
    boundary memlet. The Python backend must allocate it as an alias, including
    the write direction, rather than rejecting the remaining View descriptor.
    """
    from dace.transformation.passes.remove_views import RemoveViews
