...
class ExpandGemmPure(ExpandTransformation):
    ...
    def make_sdfg(node, parent_state, parent_sdfg, rowwise=False):
        ...
        # Runtime coefficients are represented as length-one inputs and are
        # composed with their matching compile-time properties.
...
class ExpandGemmCuPy(ExpandTransformation):
    ...
    def expansion(node: 'Gemm', state: SDFGState, sdfg: SDFG) -> SDFG:
        # Keep raw connector views and use the shared matrix view for CuPy.
        # This keeps the computation two-dimensional without losing outer shape.
        adata, bdata, cdata = _get_matmul_operands(node, state, sdfg)
        _, outer_array_a, base_a, _ = _matrix_operand(adata)
        _, outer_array_b, base_b, _ = _matrix_operand(bdata)
        _, outer_array_c, base_c, _ = _matrix_operand(cdata)
        shape_a, strides_a = adata[2], adata[3]
        shape_b, strides_b = bdata[2], bdata[3]
        shape_c, strides_c = cdata[2], cdata[3]
        ...
        nsdfg.add_array('_c', shape_c, dtype_c, strides=strides_c, storage=outer_array_c.storage)

        scalars = _coeff_conn_descs(node, state, sdfg)
        for conn, desc in scalars.items():
            scalar_desc = dc(desc)
            scalar_desc.transient = False
            nsdfg.add_datadesc(conn, scalar_desc)
        rt_alpha = '_alpha' in scalars
        rt_beta = '_beta' in scalars
        ...
        storage_c = outer_array_c.storage
        ...
        alpha = node.alpha
        if rt_alpha:
            alpha_expr = cupy_in_wrap('__alpha', scalars['_alpha'].storage)
            if equal_valued(1, alpha):
                code_lines.append(f'__result = ({alpha_expr}) * cupy.matmul(__a_t, __b_t)')
            else:
                code_lines.append(f'__result = {symstr(alpha)} * ({alpha_expr}) * cupy.matmul(__a_t, __b_t)')
        ...
        beta = node.beta
        has_cin = rt_beta or (not equal_valued(0, beta) and node.cin)

        if has_cin:
            cin_in = cupy_in_wrap('__cin', storage_c)
            if len(shape_c) != 2:
                cin_in = f'({cin_in}).reshape({_tuple_str(base_c)})'
            if rt_beta:
                beta_expr = cupy_in_wrap('__beta', scalars['_beta'].storage)
                if equal_valued(1, beta):
                    code_lines.append(f'__result = __result + ({beta_expr}) * {cin_in}')
                else:
                    code_lines.append(f'__result = __result + {symstr(beta)} * ({beta_expr}) * {cin_in}')
            elif equal_valued(1, beta):
        ...
        for conn in scalars:
            scalar_read = nstate.add_read(conn)
            nstate.add_edge(scalar_read, None, tasklet, '_' + conn, dace.Memlet.from_array(conn, nsdfg.arrays[conn]))
