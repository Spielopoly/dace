...
class CuTilePipeline(ppl.Pass):
    ...
    def _simplify(self, sdfg: SDFG) -> None:
        ...
        self._restore_masked_copy_strides(sdfg)

    def _restore_masked_copy_strides(self, sdfg: SDFG) -> None:
        """Restore implicit strides on compressed access-to-access copy memlets.

        Some preprocessing paths compress affine-strided copies (e.g. 0:N:s)
        into 0:K contiguous subsets where K is the accessed cardinality.
        In masked tile-op scopes this can break selfwrite copy-back semantics.
        """
        # TODO: This seems like a workaround than real fix that should be implemented somewhere else
        for state in sdfg.all_states():
            has_symbolic_masked_tile = any(type(n).__name__ == 'TileSymbolicMaskedOpLibraryNode'
                                           for n in state.nodes())
            if not has_symbolic_masked_tile:
                continue

            for edge in state.edges():
                if not isinstance(edge.src, sdfg_nodes.AccessNode) or not isinstance(edge.dst, sdfg_nodes.AccessNode):
                    continue
                if edge.data.src_subset is None or edge.data.dst_subset is None:
                    continue
                if not isinstance(edge.data.src_subset, subsets.Range) or not isinstance(edge.data.dst_subset, subsets.Range):
                    continue
                if edge.data.src_subset != edge.data.dst_subset:
                    continue
                ...
                if src_desc is None or dst_desc is None:
                    continue
                if len(src_desc.shape) != len(edge.data.src_subset):
                    continue
                if len(dst_desc.shape) != len(edge.data.dst_subset):
                    continue

                new_ranges = []
                changed = False
                for dim, (rng, src_dim, dst_dim) in enumerate(zip(edge.data.src_subset, src_desc.shape, dst_desc.shape)):
                    start, end, step = rng
                    try:
                        start_i = int(start)
                        end_i = int(end)
                        step_i = int(step)
                        src_dim_i = int(src_dim)
                        dst_dim_i = int(dst_dim)
                    except TypeError:
                        new_ranges.append(rng)
                        continue

                    if step_i != 1 or start_i != 0 or src_dim_i != dst_dim_i:
                        new_ranges.append(rng)
                        continue

                    extent = end_i - start_i + 1
                    if extent <= 1 or src_dim_i <= extent:
                        new_ranges.append(rng)
                        continue

                    stride = (src_dim_i + extent - 1) // extent
                    if stride <= 1 or (src_dim_i + stride - 1) // stride != extent:
                        new_ranges.append(rng)
                        continue

                    new_ranges.append((0, (extent - 1) * stride, stride))
                    changed = True

                if changed:
                    new_subset = subsets.Range(new_ranges)
                    edge.data.src_subset = new_subset
                    edge.data.dst_subset = new_subset
