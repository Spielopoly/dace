...
def test_entry_canonicalization_skips_incompatible_lifts():
    """The vectorizer canonicalizes at its own entry, and must do so with
    ``semantic_lifting=False`` and ``reduction_to_wcr_map=False``.

    A lifted ``Einsum`` / ``Copy`` / ``Memset`` library node
    ...
    that rewrite, the exact ordering the canonicalize recipe documents. The privatized-WCR map is
    a CPU codegen shape that also bypasses the tile reduction lowering."""
    assert ENTRY_CANONICALIZE_KWARGS['semantic_lifting'] is False
    assert ENTRY_CANONICALIZE_KWARGS['reduction_to_wcr_map'] is False
