...
class TestMathNameMapping:
    ...
    @pytest.mark.parametrize("cast", ["int64", "dace.int64", "numpy.int64", "np.int64"])
    def test_dtype_cast(self, cast):
        code = _rewrite_cutile_tasklet_code(f"o = {cast}(a)", {})
        assert code == "o = ct.astype(a, ct.int64)"

    def test_bool_dtype_cast_alias(self):
        code = _rewrite_cutile_tasklet_code("o = dace.bool(a)", {})
        assert code == "o = ct.astype(a, ct.bool_)"
...
class TestTaintedLocalCondition:
    ...
    def test_local_assigned_from_tile_conn(self):
        code = _rewrite_cutile_tasklet_code("cond = __in1 > 0\n__out = (__in2 if cond else __in3)", {
            "__in1": "float64",
            "__in2": "float64",
            "__in3": "float64"
        })
    ...
    def test_transitively_tainted_local(self):
        code = _rewrite_cutile_tasklet_code("a = __in1 * 2\nb = a > 1\n__out = (0.0 if b else __in2)", {
            "__in1": "float64",
            "__in2": "float64"
        })
    ...
    def test_augassigned_local_tainted(self):
        code = _rewrite_cutile_tasklet_code("acc = 0\nacc += __in1\n__out = (__in2 if acc > 0 else __in3)", {
            "__in1": "int64",
            "__in2": "int64",
            "__in3": "int64"
        })
