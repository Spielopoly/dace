...
def test_lu_symbolic_views_vectorize_and_run_with_pure_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Isolate symbolic-view widening from the separate CPU reduction-to-WCR stage, which currently
    # drops the frontend's ``A_index`` producer for this LU shape. CuTile disables this stage too.
    monkeypatch.setitem(ENTRY_CANONICALIZE_KWARGS, "reduction_to_wcr_map", False)
