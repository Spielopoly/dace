...
def _get_type_string(dtype: dtypes.typeclass) -> str:
    return str(dtype.type.__name__)


def _assign_variable(stream: PythonCodeIOStream,
                     name: str,
                     type: dtypes.typeclass,
                     value: str = "None",
                     cfg: ControlFlowRegion | None = None,
                     state_id: int | None = None,
                     node: nodes.Node | None = None,
                     type_or_none: bool = False
                     ) -> None:
    if type_or_none:
        stream.write(f'{name}: {_get_type_string(type)} | None = {value}', cfg, state_id, node)
    else:
        stream.write(f'{name}: {_get_type_string(type)} = {value}', cfg, state_id, node)
...
class PythonCodeGen(PythonTargetCodeGenerator):
        ...
        for name, arg_type in args.items():
            if isinstance(arg_type, data.Scalar):
                ...
                if arg_type.storage is dtypes.StorageType.GPU_Global:
                    raise NotImplementedError("GPU global memory is not supported as a scalar argument for the Python backend.")

                self._dispatcher.defined_vars.add(name, DefinedType.Scalar, _get_type_string(arg_type.dtype))
            elif isinstance(arg_type, data.Array):
                # TODO: Pointers
                self._dispatcher.defined_vars.add(name, DefinedType.Pointer, _get_type_string(dtypes.pointer(arg_type.dtype)))
                ...
                raise NotImplementedError("Structures are not yet supported for the Python backend.")
                # TODO: Pointers
                self._dispatcher.defined_vars.add(name, DefinedType.Pointer, _get_type_string(dtypes.pointer(arg_type.dtype)))
            else:
    ...
    def __init__(self, frame_codegen: 'DaCePythonCodeGenerator', sdfg: SDFG):
        ...
        # TODO: C++ locals work very differently from Python locals
        ...
        # TODO: We really only support Sequential schedule
        ...
        # TODO: We might not need this because we will have the copy pass
    ...
    def get_generated_codeobjects(self):
        # Python target generates inline code
        return []
    ...
    def allocate_reference(self, sdfg: SDFG, cfg: ControlFlowRegion, dfg: SDFGState, state_id: int,
                           node: nodes.AccessNode, global_stream: PythonCodeIOStream, declaration_stream: PythonCodeIOStream,
        ...
        if not declared:
            _assign_variable(declaration_stream, name, nodedesc.dtype, "None", cfg, state_id, node, type_or_none=True)
            # TODO: Pointers
            typedef = _get_type_string(dtypes.pointer(nodedesc.dtype))
            self._dispatcher.declared_arrays.add(ptrname, DefinedType.Pointer, typedef)
            self._dispatcher.defined_vars.add(ptrname, DefinedType.Pointer, typedef)

    def declare_array(self, sdfg: SDFG, cfg: ControlFlowRegion, dfg: StateSubgraphView, state_id: int, node: nodes.Node,
                      nodedesc: data.Data, function_stream: PythonCodeIOStream, declaration_stream: PythonCodeIOStream) -> None:
        ...
        # TODO: Pointers (whole function)
        # TODO: Could probably get rid of most of this method, aka should combine with allocate
        name = node.root_data
        ptrname = self.ptr(name, nodedesc, sdfg)
        ...
        if (nodedesc.storage == dtypes.StorageType.CPU_Heap or nodedesc.storage == dtypes.StorageType.Register):
            ...
            declaration_stream.write(f'{name}: {typedef} | None = None', cfg, state_id, node)
            self._dispatcher.declared_arrays.add(name, DefinedType.Pointer, typedef)
            return
        elif nodedesc.storage is dtypes.StorageType.CPU_ThreadLocal:
            raise NotImplementedError("Thread-local storage is not supported for the Python backend.")
        else:
    ...
    def allocate_array(self,
        ...
        if isinstance(nodedesc, data.Structure) and not isinstance(nodedesc, data.StructureView):
            ...
            if allocate_nested_data:
                for k, v in nodedesc.members.items():
                    if isinstance(v, data.Data):
                        typedef = dtypes.pointer(v.dtype).ctype if isinstance(v, data.Array) else v.dtype.ctype
                        defined_type = DefinedType.Scalar if isinstance(v, data.Scalar) else DefinedType.Pointer
                        self._dispatcher.declared_arrays.add(f"{name}->{k}", defined_type, typedef)
                        if isinstance(v, data.Scalar):
                            # NOTE: Scalar members are already defined in the struct definition.
                            self._dispatcher.defined_vars.add(f"{name}->{k}", defined_type, typedef)
                        else:
        ...
        if isinstance(nodedesc, data.Scalar):
            ...
            allocation_stream.write(
                ...
                state_id, node)
            define_var(name, DefinedType.Pointer, typedef)
    ...
    def deallocate_array(self, sdfg: SDFG, cfg: ControlFlowRegion, dfg: StateSubgraphView, state_id: int,
                         node: nodes.AccessNode, nodedesc: data.Data, function_stream: PythonCodeIOStream,
        ...
        if isinstance(nodedesc, data.Array) and nodedesc.start_offset != 0:
            raise NotImplementedError('Start offset unsupported in the python backend for deallocation')
        ...
        if isinstance(nodedesc, (data.Scalar, data.View, data.Stream, data.Reference)):
            return
              ...
              or (nodedesc.storage == dtypes.StorageType.Register and
                  (symbolic.issymbolic(arrsize, sdfg.constants) or
                   (arrsize_bytes and ((arrsize_bytes > Config.get("compiler", "max_stack_array_size")) == True))))):
            callsite_stream.write(f"del {alloc_name}", cfg, state_id, node)
        else:
            return
    ...
    def _emit_copy(
        ...
        raise NotImplementedError("Use copy library nodes. Copying between AccessNodes are not supported in the python backend.")
    ...
    def write_and_resolve_expr(self,
        ...
        """
        # In theory one could optimize this by detecting reduction type, similar to C++ backend,
        # and emitting for example numpy.sum calls, but the python target is not supposed to be
        # high-performance, so we prioritize simplicity and only emit the lambda function as-is.
    ...
    def process_out_memlets(self,
                            ...
                            function_stream: PythonCodeIOStream,
                            skip_wcr: bool = False,
                            codegen: Optional[PythonTargetCodeGenerator] = None):
                ...
                if (is_scalar and not memlet.dynamic and not is_stream) or is_refset:
                    ...
                    if memlet.wcr is not None:
                        write_expr = codegen.write_and_resolve_expr(
                            memlet, out_local_name, in_local_name)
                    else:
    ...
    def _generate_Tasklet(self,
        ...
        outer_stream_begin = PythonCodeIOStream()
        outer_stream_end = PythonCodeIOStream()
        ...
        after_memlets_stream = PythonCodeIOStream()
    ...
    def _generate_NestedSDFG(
        ...
        nested_stream = PythonCodeIOStream()
        nested_global_stream = PythonCodeIOStream()
    ...
    def _generate_MapExit(self, sdfg: SDFG, cfg: ControlFlowRegion, dfg: StateSubgraphView, state_id: int,
                          node: nodes.MapExit, function_stream: PythonCodeIOStream, callsite_stream: PythonCodeIOStream) -> None:
        result = callsite_stream
    ...
    def generate_scope_preamble(self, sdfg, dfg_scope, state_id, function_stream, outer_stream, inner_stream):
        ...
        :param outer_stream: A `PythonCodeIOStream` object that points
                             to the code before the scope generation (e.g.,
                             before for-loops or kernel invocations).
        :param inner_stream: A `PythonCodeIOStream` object that points
    ...
    def generate_tasklet_preamble(self, sdfg, cfg, dfg_scope, state_id, node, function_stream, before_memlets_stream,
        ...
        :param function_stream: A `PythonCodeIOStream` object that will be
                                generated outside the calling code, for
                                use when generating global functions.
        :param before_memlets_stream: A `PythonCodeIOStream` object that will emit
                                      code before input memlets are generated.
        :param after_memlets_stream: A `PythonCodeIOStream` object that will emit code
