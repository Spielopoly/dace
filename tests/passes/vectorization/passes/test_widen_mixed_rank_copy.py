...
def test_direct_tile_write_is_staged_when_output_is_also_read():
    ...
    state.add_edge(
        ...
        output,
        None,
        dace.Memlet(data="output", subset="i, j:j + 32", other_subset="0:32"),
    )
    ...
    staged = staging._stage_writes_in_state(state, sdfg, ("j", ), None)

    assert staged == 1
    stores = [node for node in state.nodes() if isinstance(node, TileStore)]
    assert len(stores) == 1
    assert not any(
        isinstance(node, dace.nodes.AccessNode) and node.data == "output_tile_out" and state.in_degree(node) == 0
