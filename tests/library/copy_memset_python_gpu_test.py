...
def _make_copy_sdfg(
    ...
    state.add_node(cpy)
    state.add_edge(src_node, None, cpy, CopyLibraryNode.INPUT_CONNECTOR_NAME,
                   dace.Memlet(f'src[{src_sub}]'))
    state.add_edge(cpy, CopyLibraryNode.OUTPUT_CONNECTOR_NAME, dst_node, None,
                   dace.Memlet(f'dst[{dst_sub}]'))
...
def _make_memset_sdfg(
    ...
    state.add_node(mset)
    state.add_edge(mset, MemsetLibraryNode.OUTPUT_CONNECTOR_NAME, arr_node, None,
                   dace.Memlet(f'arr[{sub}]'))
...
def _make_an_copy_sdfg(
    ...
    state.add_edge(src_node, None, dst_node, None,
                   dace.Memlet(data='src', subset=sub, other_subset=sub))
    return sdfg
...
class TestCopyPythonGPUCrossStorage:
    """Prove that the Python expansion dispatches cross-storage copies
    via CuPy ``.set()`` (host->device) and ``.get(out=...)`` (device->host)."""
    ...
    def test_roundtrip_h2d_then_d2h(self):
        ...
        sdfg.add_array('host_in', (64,), dace.float64,
                        storage=dtypes.StorageType.CPU_Heap)
        sdfg.add_array('gpu_buf', (64,), dace.float64,
                        storage=dtypes.StorageType.GPU_Global)
        sdfg.add_array('host_out', (64,), dace.float64,
                        storage=dtypes.StorageType.CPU_Heap)
        ...
        s1.add_node(cpy1)
        s1.add_edge(src1, None, cpy1, CopyLibraryNode.INPUT_CONNECTOR_NAME,
                    dace.Memlet('host_in[0:64]'))
        s1.add_edge(cpy1, CopyLibraryNode.OUTPUT_CONNECTOR_NAME, dst1, None,
                    dace.Memlet('gpu_buf[0:64]'))
        ...
        s2.add_node(cpy2)
        s2.add_edge(src2, None, cpy2, CopyLibraryNode.INPUT_CONNECTOR_NAME,
                    dace.Memlet('gpu_buf[0:64]'))
        s2.add_edge(cpy2, CopyLibraryNode.OUTPUT_CONNECTOR_NAME, dst2, None,
                    dace.Memlet('host_out[0:64]'))
...
class TestCopyPythonGPUSameStorage:
    ...
    def test_device_to_device_1d(self):
        ...
        _require_cupy()
        sdfg, libnode = _make_copy_sdfg(
            'gpu_d2d_1d', (64,), dace.float64,
            dtypes.StorageType.GPU_Global, dtypes.StorageType.GPU_Global)
        sdfg.validate()
        sdfg.expand_library_nodes()
        assert libnode.implementation == 'Python'
        sdfg.validate()
...
class TestMemsetPythonGPU:
    ...
    def test_gpu_memset_slice(self):
        ...
        _require_cupy()
        sdfg, libnode = _make_memset_sdfg(
            'gpu_mset_slice', (64,), dace.float64,
            dtypes.StorageType.GPU_Global, subset='10:50')
        sdfg.validate()
        sdfg.expand_library_nodes()
        assert libnode.implementation == 'Python'
        sdfg.validate()
...
class TestInsertExplicitCopiesPythonBackend:
    ...
    def test_cpu_same_storage(self):
        ...
        sdfg = _make_an_copy_sdfg(
            'iec_cpu', (32,), dace.float64,
            dtypes.StorageType.CPU_Heap, dtypes.StorageType.CPU_Heap)
        sdfg.backend = dtypes.BackendLanguage.Python
...
class TestEdgeCases:
    ...
    def test_3d_h2d(self):
        ...
        _require_cupy()
        sdfg, libnode = _make_copy_sdfg(
            'gpu_3d_h2d', (4, 8, 16), dace.float64,
            dtypes.StorageType.CPU_Heap, dtypes.StorageType.GPU_Global)
        sdfg.validate()
        sdfg.expand_library_nodes()
        assert libnode.implementation == 'Python'
    ...
    def test_gpu_memset_3d(self):
        ...
        _require_cupy()
        sdfg, libnode = _make_memset_sdfg(
            'gpu_mset_3d', (4, 5, 6), dace.float64,
            dtypes.StorageType.GPU_Global)
        sdfg.validate()
        sdfg.expand_library_nodes()
        assert libnode.implementation == 'Python'
